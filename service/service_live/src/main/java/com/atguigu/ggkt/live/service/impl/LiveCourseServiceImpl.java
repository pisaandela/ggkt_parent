...
public class LiveCourseServiceImpl extends ServiceImpl<LiveCourseMapper, LiveCourse> implements LiveCourseService {
    ...
    public void saveLive(LiveCourseFormVo liveCourseFormVo) {
        ...
        String res = null;
        try {
            res = mtCloudClient.courseAdd(liveCourse.getCourseName(), teacher.getId().toString(), new DateTime(liveCourse.getStartTime()).toString("yyyy-MM-dd HH:mm:ss"), new DateTime(liveCourse.getEndTime()).toString("yyyy-MM-dd HH:mm:ss"), teacher.getName(), teacher.getIntro(), options);
        } catch (Exception e) {
            e.printStackTrace();
            throw new GgktException(20001,"failed");
        }
